...
view(d_part_A) #Viewing the data-set (d_part_A)
str(d_part_A) #Reviewing the structure of the data-set

#Creating table One/Summary statistics table for continuous variables, to identify potential abnormalities
names(d_part_A) #Looking up variable names
...
Table1_ <- CreateTableOne(data = d_part_A,strata = strata, vars = cv) 
print(Table1_) 
...
nv <- d_part_A %>% select('pregnancies','glucose','insulin','skin_thickness','blood_pressure', "bmi","diabetes_genetic_score","age")
...
for (var in colnames(nv)) {
  nv_hist <- ggplot(d_part_A, aes_string(x = var)) + geom_histogram(fill = "lightpink", color = "black", bins = 30) +
...
for (var in colnames(nv)) {
  print( ggplot(d_part_A, aes_string(sample = var)) + stat_qq(color = "blue") + stat_qq_line(color = "red") +
...
for (var in colnames(nv)) {
  print(
    ggplot(d_part_A, aes_string(y = var)) +
...
lm_1 <- glm(diabetes ~ bmi + glucose + diabetes_genetic_score + age 
            + pregnancies + blood_pressure +  skin_thickness + insulin, data = d_part_A, 
            family = binomial)
...
Lm_2 <- glm(diabetes ~ bmi + glucose + diabetes_genetic_score + age + pregnancies + skin_thickness, data = d_part_A, family = binomial)
...
th <- 4 / nrow(d_part_A)  # 4/n was deemed overly sensitive, flagging too many points
th_2 <- 20 / nrow(d_part_A) #Seemed better healthcare context of the data
...
ipc <- which(cd > ct)
print(ipc)
ipcc <- d_part_A[ipc, ] #Sub-setting influential points 
